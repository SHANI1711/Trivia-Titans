...
const Leaderboard = () => {
	...
	const showHighestScoreAlert = (highestScoringPlayer) => {
		if (highestScoringPlayer) {
			...
			const alertShownBefore = localStorage.getItem(
				'HIGHEST_SCORE_ALERT_SHOWN'
			);

			if (
				!alertShownBefore ||
				JSON.parse(alertShownBefore)[playerID] !== true
			) {
				...
				const updatedAlerts = {
					...(alertShownBefore ? JSON.parse(alertShownBefore) : {}),
					[playerID]: true,
				};
	...
	useEffect(() => {
		...
		const fetchData = async () => {
			try {
				...
				const response = await axios.post('leaderboard-api');
				console.log('Data :', response);
				const data = JSON.parse(response.data.body); // Parse the JSON data
				console.log('Data :', data);

				setLeaderboardData(data);
				const highestScoringPlayer = aggregatePlayerData(data).find(
					(item) => item.highestScore
				);
				showHighestScoreAlert(highestScoringPlayer);
				setLoading(false);
			} catch (error) {
							...
							<Table>
								...
								<TableBody>
									{playerData.map((item, index) => (
										<TableRow
											key={index}
											className={item.highestScore ? 'highest-score' : ''}
										>
											<TableCell>{item.playerID}</TableCell>
											<TableCell>{item.score}</TableCell>
											<TableCell>{item.gamePlayedTimes}</TableCell>
											<TableCell>{Array.from(item.teams).join(', ')}</TableCell>
										</TableRow>
									))}
								</TableBody>
							</Table>
