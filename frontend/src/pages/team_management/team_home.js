...
const TeamHome = () => {
	...
	const [emailId, setEmailId] = useState(
		window.localStorage.getItem('LOGGED_IN_USER_USER_EMAIL')
	);
	...
	const handleLeaveTeam = async () => {
		try {
			...
			const response = await axios.put('leave-team-api', payload);
	...
	async function handleRemoveMember(email) {
		...
		const response = await axios.put('remove-member-api', payload);
		const { body } = response.data;
		alert(`${email} :${body}`);
		fetchData();
	}
	...
	async function handlePromoteUser(email) {
		...
		const response = await axios.put('promote-admin-api', payload);
		const { body } = response.data;
		alert(`${email} :${body}`);
		fetchData();
	}
	...
	const fetchStats = async () => {
		try {
			...
			const response = await axios.post('fetch-stats-api', payload);
			const { wins, loss, games_played, points } = JSON.parse(
				response.data.body
			);
	...
	const handleCreateTeam = async () => {
		const url = 'completions-api';
		const prompt =
			'Create a name for  a highly competetive team participating in quiz competetion'; // Customize the prompt
		try {
			const response = await axios.post(
				...
				},
				{
					headers: {
						Authorization:
							'Bearer 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',
			...
			const teamCreationUrl = 'createTeamFunction-api';
			...
			alert(
				`${body.message} with team id : ${body.team_id} and Team Name : ${body.teamName}`
			);
	...
	const fetchData = async () => {
		try {
			...
			const response = await axios.post('fetchTeamList-api', payload);
